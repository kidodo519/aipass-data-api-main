...
API_AUTH_URL=https://api.dev.aipass.jp/public/oauth/token
...
- If you only have an email/password (no API token), set `API_EMAIL` (or `API_USERNAME`) and `API_PASSWORD` and leave `API_TOKEN` empty. The script will request an access token from `API_AUTH_URL`.
